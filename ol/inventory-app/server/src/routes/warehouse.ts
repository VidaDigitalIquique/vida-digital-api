...
router.post("/locations", async (req, res) => {
  try {
    ...
    if (!isLocationCode(code)) return res.status(400).json({ error: "code invÃƒÆ’Ã‚Â¡lido" });
...
router.get("/counts", async (req, res) => {
  ...
  ...r,
  countedBoxes: r.countedBoxes != null ? Number(r.countedBoxes) : null,
}));
res.json(norm);
  } catch (e: any) {
...
router.post("/counts", async (req, res) => {
  try {
    ...
    if (!isLocationCode(locationCode)) return res.status(400).json({ error: "locationCode invÃƒÆ’Ã‚Â¡lido" });
    if (!Number.isFinite(countedBoxesNum)) return res.status(400).json({ error: "countedBoxes invÃƒÆ’Ã‚Â¡lido" });
...
router.put("/counts/:id", async (req, res) => {
  try {
    ...
    if (req.body?.countedBoxes !== undefined) {
      ...
      if (!Number.isFinite(n)) return res.status(400).json({ error: "countedBoxes invÃƒÆ’Ã‚Â¡lido" });
  ...
  try {
    ...
    const sql = `
      SELECT
        ...
        c."countedBoxes"::numeric AS "countedBoxes",
        c."countedAt" AS "countedAt",
        c.note AS "note",
        loc.code AS "locationCode",
        loc.name AS "locationName"
      FROM app_physical_count c
      LEFT JOIN app_location loc ON loc.id = c.location_id
      WHERE c."productCode" = $1
      ORDER BY c."countedAt" DESC NULLS LAST, c.id DESC
      LIMIT $2
    `;
    const { rows } = await pool.query(sql, [productCode, limit]);
    const out = rows.map(r => ({ ...r, countedBoxes: r.countedBoxes != null ? Number(r.countedBoxes) : null }));
    res.json(out);
  } catch (e: any) {
    res.status(500).json({ error: e.message || "recent counts error" });
  }
});
